...
def get_all_links(url, session=None):
    ...
    try:
        ...
        links = []
        # Keywords that might indicate professor-related pages
        professor_keywords = ['faculty', 'professor', 'staff', 'people', 'members', 'researchers', 'team']
        ...
        for anchor in soup.find_all('a', href=True):
            ...
            # Prioritize links that might lead to professor pages
            priority = 0
            if any(keyword in anchor_text for keyword in professor_keywords):
                priority = 2
                logger.info(f"Found potential professor-related link: {href} - Text: {anchor_text}")
        ...
        for link, priority in sorted(links, key=lambda x: x[1], reverse=True):
            if link not in seen:
                seen.add(link)
...
def is_professor_webpage(url, session=None, client=None):
    ...
    try:
        ...
        professor_keywords = ['professor', 'faculty', 'dr.', 'ph.d', 'phd', 'research interests', 
                            'publications', 'curriculum vitae', 'cv', 'teaching', 
                            'associate professor', 'assistant professor', 'scholar']
        keyword_matches = [kw for kw in professor_keywords if kw.lower() in text.lower() or kw.lower() in title.lower()]
        ...
        llm_response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are an AI that determines if a webpage belongs to a professor at a university or research institution. Look for indicators like academic titles (Professor, Dr., Ph.D.), research interests, publications, courses taught, and academic affiliations."},
                {"role": "user", "content": f"Based on the following webpage details, determine if this is a personal or professional page of a professor, faculty member, or academic researcher at a university or research institution. Answer only YES or NO.\n\n{important_elements}"}
            ]
        )
...
def get_research_interests(url, session=None, client=None):
    ...
    try:
        ...
        research_sections = []
        research_keywords = ['research', 'interests', 'projects', 'expertise', 'publications']
        ...
        if research_sections:
            research_content = '\n\n'.join(f"{section['header']}: {section['content'][:500]}" 
        ...
        llm_response = client.chat.completions.create(
            model="doubao-1-5-pro-32k-250115",
            messages=[
                {"role": "system", "content": "You are an AI that extracts and summarizes a professor's research interests from their webpage. Your task is to provide detailed summaries that capture specific research areas, methodologies, and even subtle differences in research directions."},
                {"role": "user", "content": f"Based on the following professor's webpage content, provide a detailed summary of their research interests. Include specific topics, methodologies, and capture any nuanced differences in research directions. Make the summary comprehensive while remaining focused on their core research areas:\n\nPage Title: {title}\n\n{research_content}"}
            ]
        )
...
def analyze_webpage_links(start_url, api_key, max_links=50, max_workers=5):
    """Main function to analyze links from a website."""
    ...
    # Get all links from the starting URL
    all_links = get_all_links(start_url, session)[:max_links]
    logger.info(f"Found {len(all_links)} links to analyze.")
    
    if not all_links:
        logger.warning("No links found to analyze. Check if the website is accessible or has changed structure.")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_link = {executor.submit(process_link, link, session, client): link for link in all_links}
            ...
            try:
                ...
                if result:
                    ...
                    if result["Is Professor Page"] == "Yes":
                        ...
                        logger.info(f"Research Interests: {result['Research Interests']}")
            except Exception as e:
    ...
    logger.info(f"Analysis complete. Found {len(professors_df)} professor pages out of {len(results)} total links analyzed.")
    ...
    logger.info(f"Results Summary: {yes_count} professor pages, {no_count} non-professor pages, {error_count} errors")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Analyze a website to find professor pages and research interests.')
    ...
    parser.add_argument('--max-links', type=int, default=30, help='Maximum number of links to analyze')
    parser.add_argument('--workers', type=int, default=5, help='Number of worker threads')
    parser.add_argument('--output', default='professor_analysis_results.csv', help='Output CSV filename')
    args = parser.parse_args()
    
    # Run the analysis
    results_df = analyze_webpage_links(args.url, args.api_key, max_links=args.max_links, max_workers=args.workers)
